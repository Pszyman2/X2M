...
function [dataSubjects,dataSubjectsDetailed] = x2mGetSubjects(servers,upTo)
...
for i = 1:size(servers,2)
    ...
    url = [ server '/data/archive/subjects' ];
    try %http errors log
            dataQuery = webread(url, options);                              % default - get 
            x2mAddToLog('subject-query',server,user,'OK','','','','',dataQuery.ResultSet.totalRecords,'');
...
%                   end
                  for k=1:upTo
                      ...
                      x2mAddToLog('subject-innerQuery',server,user,'OK','','','','','','');
                  ...
                  x2mAddToLog('subject-innerQuery',server,user,me.message,'','','','','','');  
                end
